...
class WordPress_Sniffs_Arrays_ArrayDeclarationSpacingSniff extends Squiz_Sniffs_Arrays_ArrayDeclarationSniff {
	...
	protected function fix_associative_array( PHP_CodeSniffer_File $phpcsFile, $arrayStart, $arrayEnd ) {
		...
		$next_to_check = $arrayStart;
		while ( false !== ( $next_to_check = $phpcsFile->findNext( array( T_COMMA, T_OPEN_SHORT_ARRAY ), ( $next_to_check + 1 ), $arrayEnd ) ) ) {
			// Ignore anything within short array definition brackets.
			// Necessary as the nesting level in that case is still the same.
			if ( 'T_OPEN_SHORT_ARRAY' === $tokens[ $next_to_check ]['type']
				&& ( isset( $tokens[ $next_to_check ]['bracket_opener'] )
					&& $tokens[ $next_to_check ]['bracket_opener'] === $next_to_check )
				&& isset( $tokens[ $next_to_check ]['bracket_closer'] )
			) {
				$next_to_check = $tokens[ $next_to_check ]['bracket_closer'];
				continue;
			}

			// Ignore comma's at a lower nesting level.
			if ( 'T_COMMA' === $tokens[ $next_to_check ]['type']
				&& isset( $tokens[ $next_to_check ]['nested_parenthesis'] )
				&& count( $tokens[ $next_to_check ]['nested_parenthesis'] ) !== $nesting_count
			) {
				continue;
			}

			$phpcsFile->fixer->addNewline( $next_to_check );
			if ( T_WHITESPACE === $tokens[ ( $next_to_check + 1 ) ]['code'] ) {
				$phpcsFile->fixer->replaceToken( ( $next_to_check + 1 ), $value_indentation );
			} else {
				$phpcsFile->fixer->addContentBefore( ( $next_to_check + 1 ), $value_indentation );
			}
		}
